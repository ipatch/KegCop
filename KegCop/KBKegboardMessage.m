...
//
// TODO - capin
// - clearly define what class does
...
+ (id)messageWithId:(NSUInteger)messageId payload:(char *)payload length:(NSUInteger)length timeStamp:(NSTimeInterval)timeStamp {
    ...
    switch (messageId) {
        case KB_MESSAGE_ID_HELLO:
            ...
            // capin - removed "autorelease" from end of "message = ..." to make statement work with ARC
...
- (BOOL)parsePayload:(char *)data forTag:(NSUInteger)tag length:(NSUInteger)length {
    switch (tag) {
        case 0x01:
            ...
            // capin - removed "retain" from end of "_boardName = ..." to make statement work with ARC
...
- (void)dealloc {
    ...
    // capin - commented out the below lines of code to make class work with ARC
...
- (BOOL)parsePayload:(char *)data forTag:(NSUInteger)tag length:(NSUInteger)length {
    switch (tag) {
        case 0x01:
            ...
            // capin - removed "retain" from "_meterName = ..." to make ARC compliant
...
- (void)dealloc {
    ...
    // capin - commented out below lines of code to make ARC compliant
...
- (BOOL)parsePayload:(char *)data forTag:(NSUInteger)tag length:(NSUInteger)length {
    switch (tag) {
        case 0x01:
            // capin - removed "retain" from "_sensorName = ..." statement for ARC compliance
...
- (BOOL)parsePayload:(char *)data forTag:(NSUInteger)tag length:(NSUInteger)length {
    switch (tag) {
        case 0x01:
            ...
            // capin - removed "retain" from "_outputName = ..." statement for ARC compliance
